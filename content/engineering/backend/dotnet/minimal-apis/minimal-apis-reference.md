---
title: VSA with Minimal APIs Broad Reference
description: Complete implementation examples for Vertical Slices with Minimal APIs in the Flowsy ecosystem.
type: reference
audience: Backend C# developers needing detailed examples.
canonical: false
canonicalSource: /engineering/backend/dotnet/minimal-apis/
---

# VSA with Minimal APIs

> [!IMPORTANT]
> Prefer the progressive Minimal APIs path under [VSA with Minimal APIs](/engineering/backend/dotnet/minimal-apis/). This page keeps complete examples.
...
This page is the C#/.NET implementation companion to the technology-agnostic [VSA concepts](/engineering/backend/architecture/vertical-slice-architecture). Start with the concepts when deciding slice boundaries, then use this page for concrete files, libraries and code.
...
For HTTP API maturity, status codes and RFC 9457 Problem Details, see [HTTP API Design](/engineering/backend/api/http-api-design).
...
Apply the broader [Error Handling](/engineering/backend/reliability/error-handling) guidance inside the slice:
...
Do not create this pair by habit. See [Domain-Driven Design](/foundations/domain-modeling/domain-driven-design-reference) for the full DCB design principle and implementation path guidance.
...
- [VSA: Concepts](/engineering/backend/architecture/vertical-slice-architecture) — principles and folder structure.
- [C# Conventions](/engineering/backend/dotnet/csharp) — naming and general guidelines.
- [Background Services](/engineering/backend/dotnet/background-services/) — event consumption from workers.

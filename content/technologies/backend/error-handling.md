...
This guide is language- and framework-agnostic. Use it to decide where validation belongs, when state may change, how infrastructure errors are translated and how delivery adapters expose failures. For HTTP response contracts, status codes and RFC 9457 Problem Details, see [HTTP API Design](./api-design.md).
